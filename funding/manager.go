...
func (f *Manager) waitForFundingConfirmation(
	...
	// setConfirmationHeight is a helper that marks the confirmation height
	// of the channel if it hasn't been set yet. It returns false if an
	// error occurs.
	setConfirmationHeight := func(height uint32) bool {
		if completeChan.ConfirmationHeight == 0 {
			err := completeChan.MarkConfirmationHeight(height)
			if err != nil {
				log.Errorf("failed to update confirmed state "+
	...
	for {
		...
		case updDetails, ok := <-confNtfn.Updates:
			...
			if !setConfirmationHeight(updDetails.BlockHeight) {
				return
			}
		...
		case confDetails, ok := <-confNtfn.Confirmed:
			...
			if !setConfirmationHeight(confDetails.BlockHeight) {
				return
			}
