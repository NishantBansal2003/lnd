...
func (fn *fuzzNetwork) sendRemoteReplyChannelRange(offset int) int {
	...
	if !hasEnoughData(fn.data, offset, 11) {
		return len(fn.data)
	}
	...
	firstBlockHeight := getUint32(fn.data[currentOffset : currentOffset+4])
	numBlocks := getUint32(fn.data[currentOffset+4 : currentOffset+8])
	currentOffset += 8

	complete := fn.data[currentOffset]
	currentOffset++

	var scidsList []lnwire.ShortChannelID
	if hasEnoughData(fn.data, currentOffset, 1) {
		scidCount := int(fn.data[currentOffset])
		currentOffset++
	...
	if hasEnoughData(fn.data, currentOffset, 1) {
		timestampCount := int(fn.data[currentOffset])
		currentOffset++

		for range timestampCount {
			if !hasEnoughData(fn.data, currentOffset, 8) {
				break
			}
...
// connections, and other protocol interactions. It explores various message
// sequences and peer behaviors to uncover bugs and state violations.
